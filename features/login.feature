...
Feature: Login to the application
  ...
  Scenario Outline: Verify users cannot login with invalid credentials
    ...
    Examples:
      ...
      | libraray@student | password123      |
      | library@cydeo    | libraryPassword  |

  @lib-01-03
  # TODO: Verify users are not allowed to login without any credentials
  Scenario Outline: Verify that users are not allowed to login without any credentials
    When user clicks the login button
    Then user should see "This field is required." error pop-up message